...
type ModelPayloadStyleStable struct {
	...
	Special map[string]any `json:"special,omitempty"` // Note: Swagger defines as object with additionalProperties: object
...
type InterrogationSubmitInput struct {
	...
	Result map[string]any `json:"result,omitempty"` // Swagger defines as string, but likely should be object matching InterrogationFormResult
...
type ModelStylePatchKobold struct {
	...
	// Extra information about this style.
	Info *string `json:"info,omitempty"` // MinLen: 1, MaxLen: 1000
	// The prompt template which will be sent to Stable Diffusion to generate an image. The user's prompt will be injected into this. This argument MUST include a '{p}' which specifies the part where the user's prompt will be injected.
